...
void main()
{
	...
	vec4 vertexPos = (v0 * boneWeightIn.x + v1 * boneWeightIn.y + v2 * boneWeightIn.z + v3 * boneWeightIn.w);
	...
	vec4 normalDirection = (n0 * boneWeightIn.x + n1 * boneWeightIn.y + n2 * boneWeightIn.z + n3 * boneWeightIn.w);
	#else
	vec4 vertexPos = vec4(vertexIn, 1.0);
	vec4 normalDirection = vec4(normalIn, 0.0);
	#endif

	vec4 viewSpace = camera.viewMatrix * modelPosition.matrix * vec4(vertexPos);
	vec4 projected = camera.projectionMatrix * viewSpace;

	vertex = vertexPos.xyz;
	normal = normalDirection.xyz;
	texCoord = texCoordIn;
