...
public class VoxelChest extends Voxel {
	...
	public VoxelChest(VoxelDefinition type) {
		...
		frontTexture = getStore().getTextures().get(getName() + "_front");
		sideTexture = getStore().getTextures().get(getName() + "_side");
		topTexture = getStore().getTextures().get(getName() + "_top");
	}
	...
	private Inventory getInventory(ChunkCell context) {
		VoxelComponent comp = context.getComponents().getVoxelComponent("chestInventory");
	...
	public void whenPlaced(FreshChunkCell cell) {
		...
		VoxelInventoryComponent component = new VoxelInventoryComponent(cell.getComponents(), 10, 6);
