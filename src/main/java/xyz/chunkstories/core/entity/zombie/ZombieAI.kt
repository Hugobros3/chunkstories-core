...
class ZombieAI(entity: EntityZombie) : AggressiveAI<EntityZombie>(entity, targets) {
    override fun bark() {
        entity.world.soundManager.playSoundEffect("sounds/entities/zombie/grunt.ogg", SoundSource.Mode.NORMAL, entity.location, (0.9 + Math.random() * 0.2).toFloat(), 1.0f)
    }

    override fun aggroBark() {
        entity.world.soundManager.playSoundEffect("sounds/entities/zombie/grunt.ogg", SoundSource.Mode.NORMAL, entity.location, (1.5 + Math.random() * 0.2).toFloat(), 1.5f)
    }
