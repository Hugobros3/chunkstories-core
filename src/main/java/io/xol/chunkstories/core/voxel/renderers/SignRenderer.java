...
public class SignRenderer implements VoxelDynamicRenderer {
	...
	public void renderVoxels(RenderingInterface renderer, IterableIterator<ChunkCell> renderableEntitiesIterator) {
		...
		for (ChunkCell context : renderableEntitiesIterator)// .getElementsInFrustrumOnly())
		{
			...
			// bake sign mesh
			if (signTextComponent.cachedText == null
					|| !signTextComponent.cachedText.equals(signTextComponent.getSignText())) {
				// entitySign.renderData = new
				// TextMeshObject(entitySign.signText.getSignText());
				signTextComponent.cachedText = signTextComponent.getSignText();
				signTextComponent.renderData = renderer.getFontRenderer()
						.newTextMeshObject(renderer.getFontRenderer().defaultFont(), signTextComponent.cachedText);
			}
