...
public class ParticleFireLight extends ParticleTypeHandler {
	...
	public void forEach_Physics(World world, ParticleData data) {
		...
		/*
		 * b.setX((float) (b.x() + b.vel.x())); b.setY((float) (b.y() + b.vel.y()));
		 * b.setZ((float) (b.z() + b.vel.z()));
		 */

		/*
		 * if (!((WorldImplementation) world).checkCollisionPoint(b.x(), b.y(), b.z()))
		 * b.vel.setY(b.vel.y() + 0.02/60.0); else b.vel.set(0d, 0d, 0d);
		 */
	...
	public ParticleTypeRenderer getRenderer(ParticlesRenderer renderer) {
		return new ParticleTypeRenderer(renderer) {
			...
			public void beginRenderingForType(RenderingInterface renderer) {
				...
				renderer.bindTexture2D("colorTempSampler",
			...
			public void forEach_Rendering(RenderingInterface renderingContext, ParticleData data) {
				...
				renderingContext.getLightsRenderer()
						.queueLight(new Light(new Vector3f(1.0f, 252f / 255f, 1 / 255f).mul(5f),
