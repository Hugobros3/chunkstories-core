...
public class WorldEffects {
	public static void createFireball(World world, Vector3d center, double radius, double debrisSpeed, float f) {
		...
		world.getSoundManager().playSoundEffect("./sounds/environment/kboom.ogg", Mode.NORMAL, center,
				(float) (0.9f + Math.random() * 0.2f), (float) (debrisSpeed * debrisSpeed * 10f), 1, 150);
	}
	...
	public static void createFireballFx(World world, Vector3d center, double radius, double debrisSpeed, float f) {
		for (int z = 0; z < 250 * f; z++) {
			Vector3d lol = new Vector3d(Math.random() * 2.0 - 1.0, Math.random() * 2.0 - 1.0,
					Math.random() * 2.0 - 1.0);
			lol.normalize();
		...
		world.getParticlesManager().spawnParticleAtPositionWithVelocity("fire_light", center,
				new Vector3d(1, 0, 0).normalize().mul(debrisSpeed * 1.5f));
	}
}
