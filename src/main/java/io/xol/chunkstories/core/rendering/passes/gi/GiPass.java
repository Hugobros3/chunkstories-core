...
public class GiPass extends RenderPass {
	...
	public GiPass(RenderPasses pipeline, String name, String[] requires, String[] exports, ShadowPass sunShadowPass) {
		...
		float giScale = 2.0f;
		accumulationA = pipeline.getRenderingInterface().newTexture2D(TextureFormat.RGBA_16F,
				(int) (worldRenderer.getWindow().getWidth() / giScale),
				(int) (worldRenderer.getWindow().getHeight() / giScale));
		accumulationB = pipeline.getRenderingInterface().newTexture2D(TextureFormat.RGBA_16F,
				(int) (worldRenderer.getWindow().getWidth() / giScale),
				(int) (worldRenderer.getWindow().getHeight() / giScale));

		confidenceA = pipeline.getRenderingInterface().newTexture2D(TextureFormat.RED_16F,
				(int) (worldRenderer.getWindow().getWidth() / giScale),
				(int) (worldRenderer.getWindow().getHeight() / giScale));
		confidenceB = pipeline.getRenderingInterface().newTexture2D(TextureFormat.RED_16F,
				(int) (worldRenderer.getWindow().getWidth() / giScale),
				(int) (worldRenderer.getWindow().getHeight() / giScale));

		zBufferA = pipeline.getRenderingInterface().newTexture2D(TextureFormat.DEPTH_RENDERBUFFER,
				(int) (worldRenderer.getWindow().getWidth() / giScale),
				(int) (worldRenderer.getWindow().getHeight() / giScale));
		zBufferB = pipeline.getRenderingInterface().newTexture2D(TextureFormat.DEPTH_RENDERBUFFER,
				(int) (worldRenderer.getWindow().getWidth() / giScale),
				(int) (worldRenderer.getWindow().getHeight() / giScale));

		fboAccumulationA = pipeline.getRenderingInterface().getRenderTargetManager().newConfiguration(zBufferA,
				accumulationA, confidenceA);
		fboAccumulationB = pipeline.getRenderingInterface().getRenderTargetManager().newConfiguration(zBufferB,
				accumulationB, confidenceB);
	...
	public void render(RenderingInterface renderer) {
		...
		if (shadowPass != null) {
			...
			giShader.setUniform1f("shadowMapResolution",
		...
		if (cameraPosition.distance(oldCameraPosition) != 0.0f
				|| cameraDirection.distance(oldCameraDirection) != 0.0f) {
			// System.out.println("moved! : " + cameraPosition.distance(oldCameraPosition) +
			// "or " + cameraDirection.distance(oldCameraDirection));
		...
		giShader.setUniform1f("wetness",
				worldRenderer.getWorld().getGenerator().getEnvironment().getWorldWetness(cameraPosition));
