...
public class DefaultSkyRenderer implements SkyRenderer {
	...
	public Vector3f getSunPosition() {
		...
		Vector3f lookAt = new Vector3f(0.5f + (float) (Math.sin(sunElevation) * Math.cos(sunAzimuth)),
				(float) (Math.sin(sunAzimuth)), 0.3f + (float) (Math.cos(sunElevation) * Math.cos(sunAzimuth)));
	...
	public void render(RenderingInterface renderingContext) {
		...
		if (starsVertexBuffer == null) {
			...
			for (int i = 0; i < NB_STARS; i++) {
				Vector3f star = new Vector3f((float) Math.random() * 2f - 1f, (float) Math.random(),
