...
public class ItemMiningTool extends Item implements MiningTool {
	...
	public void tickInHand(Entity owner, ItemPile itemPile) {
		/*
		 * World world = owner.getWorld(); if (owner instanceof EntityControllable &&
		 * owner instanceof EntityWorldModifier) { EntityControllable entityControllable
		 * = (EntityControllable) owner; Controller controller =
		 * entityControllable.getController();
		 * 
		 * if (controller != null && controller instanceof Player) { InputsManager
		 * inputs = controller.getInputsManager();
		 * 
		 ...
		 * 
		 * // Cancel mining if looking away or the block changed by itself if (lookingAt
		 * == null || (progress != null && (lookingAt.distance(progress.loc) > 0 ||
		 * !cell.getVoxel().sameKind(progress.voxel)))) { progress = null; }
		 * 
		 * if (progress == null) { // Try starting mining something if (lookingAt !=
		 * null) progress = new MiningProgress(world.peekSafely(lookingAt), this); }
		 * else { progress.keepGoing(entityControllable, controller); } } else {
		 * progress = null; }
		 * 
		 * Player player = (Player) controller; if (player.getContext() instanceof
		 * ClientInterface) { Player me = ((ClientInterface)
		 * player.getContext()).getPlayer(); if (me.equals(player)) { myProgress =
		 * progress; } } } }
		 */
	...
	/*
	 * @Override public ItemRenderer getCustomItemRenderer(ItemRenderer
	 * fallbackRenderer) { return new SwingToolRenderer(fallbackRenderer); }
	 * 
	 ...
	 * 
	 * @Override public void renderItemInWorld(RenderingInterface
	 * renderingInterface, ItemPile pile, World world, Location location, Matrix4f
	 * transformation) {
	 * 
	 * MinerTrait trait = null; Inventory inv = pile.getInventory(); if(inv
	 * instanceof TraitInventory) trait =
	 * ((TraitInventory)inv).entity.traits.get(MinerTrait.class);
	 * 
	 * if(trait != null){ Matrix4f rotated = new Matrix4f(transformation);
	 * 
	 * Vector3f center = new Vector3f(0.0f, -0.0f, -100f);
	 * 
	 * rotated.translate(0.05f, -0.1f, 0f); MiningProgress progress =
	 * trait.getProgress();
	 * 
	 * if (progress != null) { long elapsed = System.currentTimeMillis() -
	 * progress.started; float elapsedd = (float) elapsed; elapsedd /= (float)
	 * animationCycleDuration;
	 * 
	 * if (elapsedd >= progress.timesSoundPlayed && elapsed > 50) {
	 * world.getSoundManager().playSoundEffect("sounds/gameplay/voxel_remove.ogg",
	 * Mode.NORMAL, progress.context.getLocation(), 1.5f, 1.0f);
	 * progress.timesSoundPlayed++; }
	 * 
	 * float swingCycle = (float) Math.sin(Math.PI * 2 * elapsedd + Math.PI);
	 * 
	 * rotated.translate(center); rotated.rotate((float) (swingCycle), 0f, 0f, 1f);
	 * 
	 * center.negate(); rotated.translate(center); }
	 * 
	 * // rotated.rotate((System.currentTimeMillis() % 100000) / 10000f, 0f, 0f,
	 * 1f); rotated.scale(2.0f);
	 * 
	 * super.renderItemInWorld(renderingInterface, pile, world, location, rotated);
	 * 
	 * } else super.renderItemInWorld(renderingInterface, pile, world, location,
	 * transformation); } }
	 */
