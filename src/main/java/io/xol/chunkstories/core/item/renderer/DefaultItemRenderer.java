...
public class DefaultItemRenderer extends ItemRenderer {
	...
	public void renderItemInInventory(RenderingInterface renderingInterface, ItemPile pile, float screenPositionX,
		...
		// this.itemType.store().parent()).textures());
		if (textures == null)
			return;
		...
		renderingInterface.getGuiRenderer().drawBoxWindowsSpaceWithSize(screenPositionX, screenPositionY, width, height,
				0, 1, 1, 0, texture, true, true, null);
	}
	...
	public void renderItemInWorld(RenderingInterface renderingInterface, ItemPile pile, World world, Location location,
		...
		handTransformation2.scale(new Vector3f(1.0f, 0.20f * pile.getItem().getDefinition().getSlotsWidth(),
				0.20f * pile.getItem().getDefinition().getSlotsHeight()));
	...
	protected void draw3DPlane(RenderingInterface renderingInterface) {
		/*
		 * if(defaultPlane != null) { defaultPlane.destroy(); defaultPlane = null; }
		 */
		// defaultPlane = null;
		if (defaultPlane == null) {
			...
			// ByteBuffer buf = BufferUtils.createByteBuffer(2 * (4 * 3 * 6 + 4 * 2 * 6 + 4
			// * 3 * 6));
			// Vertex pos data
			buf.putFloat(0.0f);
			buf.putFloat(-1.0f);
		...
		renderingInterface.bindAttribute("texCoordIn",
				defaultPlane.asAttributeSource(VertexFormat.FLOAT, 2, 0, 2 * 4 * 3 * 6));
		renderingInterface.bindAttribute("normalIn",
				defaultPlane.asAttributeSource(VertexFormat.FLOAT, 3, 0, 2 * (4 * 3 * 6 + 4 * 2 * 6)));
