...
public class BreakingBlockDecal {
	...
	public BreakingBlockDecal(MiningProgress miningProgress, RenderingInterface renderingInterface) {
		...
		BreakingBlockDecalVoxelBaker bbdvb = new BreakingBlockDecalVoxelBaker(
				((WorldClient) ctx.getWorld()).getClient().getContent(), miningProgress.loc);
	...
	class BreakingBlockDecalVoxelBaker extends BaseLayoutBaker implements VoxelBakerCubic, VoxelBakerHighPoly {
		...
		public ByteBuffer cum() {
			...
			ByteBuffer buffer = ByteBuffer.allocateDirect(4 * memesAreMyReality.size());// MemoryUtil.memAlloc(4 *
																						// memesAreMyReality.size());
	...
	public void render(RenderingInterface renderingInterface) {
		...
		renderingInterface.bindAttribute("vertexIn",
				vertexBuffer.asAttributeSource(VertexFormat.FLOAT, 3, 4 * (3 + 2), 0));
		renderingInterface.bindAttribute("texCoordIn",
				vertexBuffer.asAttributeSource(VertexFormat.FLOAT, 2, 4 * (3 + 2), 4 * 3));
